...
const Login = ({ onLogin }) => {
  ...
  <h2 className="title has-text-white">{t('Connexion')}</h2>
  <form onSubmit={handleSubmit}>
          ...
          {error && <div className="notification is-danger" style={{marginTop:8}}>{t(error)}</div>}
          <button className="button is-primary" type="submit">{t('Se connecter')}</button>
          <button className="button is-link ml-2" type="button" onClick={() => navigate('/register')}>{t('Créer un compte')}</button>
        </form>
        <div className="has-text-white mt-4" style={{fontSize:'0.95em'}}>
          <strong>{t('Utilisateur de test')} :</strong><br/>
          {t('Email')} : <span style={{color:'#3ec1ef'}}>client@drivncook.com</span><br/>
          {t('Mot de passe')} : <span style={{color:'#3ec1ef'}}>test</span>
        </div>
      </div>
    </section>
  );
