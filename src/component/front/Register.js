  ...
  return (
    <section className="section">
      <div className="box" style={{ maxWidth: 400, margin: '2rem auto', background: '#23272f', borderRadius: 12 }}>
        <h2 className="title has-text-white">Créer un compte</h2>
        <form onSubmit={handleSubmit}>
          ...
          <button className="button is-primary" type="submit">Créer le compte</button>
        </form>
        {error && <div className="notification is-danger mt-3">{error}</div>}
        {success && <div className="notification is-success mt-3">Compte créé ! Redirection...</div>}
      </div>
    </section>
  );
};
